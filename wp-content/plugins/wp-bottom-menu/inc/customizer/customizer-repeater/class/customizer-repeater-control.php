...
class Customizer_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'wp-bottom-menu' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'wp-bottom-menu' );
		...
		if ( file_exists( WP_BOTTOM_MENU_DIR_PATH . 'inc/customizer/customizer-repeater/inc/icons.php' ) ) {
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'customizer-repeater-admin-stylesheet', WP_BOTTOM_MENU_DIR_URL . 'inc/customizer/customizer-repeater/css/admin-style.css', array(), CUSTOMIZER_REPEATER_VERSION );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'customizer-repeater-script', WP_BOTTOM_MENU_DIR_URL . 'inc/customizer/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), CUSTOMIZER_REPEATER_VERSION, true  );

		//wp_enqueue_script( 'customizer-repeater-fontawesome-iconpicker', WP_BOTTOM_MENU_DIR_URL . 'inc/customizer-repeater/js/fontawesome-iconpicker.min.js', array( 'jquery' ), CUSTOMIZER_REPEATER_VERSION, true );

		//wp_enqueue_style( 'customizer-repeater-fontawesome-iconpicker-script', WP_BOTTOM_MENU_DIR_URL . 'inc/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), CUSTOMIZER_REPEATER_VERSION );
	}
		...
		if(!empty($array)){
						...
						*/
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','wp-bottom-menu' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_color2_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','wp-bottom-menu' ), $this->id, 'customizer_repeater_color2_control' ),
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','wp-bottom-menu' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Icon','wp-bottom-menu' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','wp-bottom-menu' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_text2_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Text','wp-bottom-menu' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if($this->customizer_repeater_link2_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','wp-bottom-menu' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Shortcode','wp-bottom-menu' ), $this->id, 'customizer_repeater_shortcode_control' ),
                        ...
                        <button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php esc_html_e( 'Delete field', 'wp-bottom-menu' ); ?>
                        </button>
            ...
            <div class="customizer-repeater-general-control-repeater-container">
                ...
                <div class="customizer-repeater-box-content-hidden">
					...
					if ( $this->customizer_repeater_subtitle_control == true ) {
						$this->input_control( array(
							'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Subtitle','wp-bottom-menu' ), $this->id, 'customizer_repeater_subtitle_control' ),
							'class' => 'customizer-repeater-subtitle-control',
							'type'  => apply_filters('customizer_repeater_input_types_filter', '', $this->id, 'customizer_repeater_subtitle_control' ),
						) );
					}
	...
	private function icon_picker_control($value = '', $show = ''){
        ...
        <div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','wp-bottom-menu'); ?>
            </span>
	...
	private function image_control($value = '', $show = ''){ ?>
        <div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' || empty( $show ) ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Image','wp-bottom-menu')?>
            </span>
            <input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
            <input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image','wp-bottom-menu' ); ?>" />
        </div>
		<?php
	}

	private function icon_type_choice($value='wpbm-custom'){ ?>
        <span class="customize-control-title">
            <?php esc_html_e('Menu type','wp-bottom-menu');?>
        </span>
		<?php /*
        <select class="customizer-repeater-image-choice">
            <option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','wp-bottom-menu'); ?></option>
            <option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','wp-bottom-menu'); ?></option>
            <option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','wp-bottom-menu'); ?></option>
		</select>
		*/
		?>

		<select class="customizer-repeater-image-choice">
            <option value="wpbm-custom" <?php if($value == "wpbm-custom") echo "selected"; ?> ><?php esc_html_e('Custom','wp-bottom-menu'); ?></option>
            <option value="wpbm-homepage" <?php if($value == "wpbm-homepage") echo "selected"; ?> ><?php esc_html_e('Home Page','wp-bottom-menu'); ?></option>
            <option value="wpbm-woo-cart" <?php if($value == "wpbm-woo-cart") echo "selected"; ?> ><?php esc_html_e('Woocommerce Cart','wp-bottom-menu'); ?></option>
			<option value="wpbm-woo-account" <?php if($value == "wpbm-woo-account") echo "selected"; ?> ><?php esc_html_e('Woocommerce Account','wp-bottom-menu'); ?></option>
            <option value="wpbm-woo-search" <?php if($value == "wpbm-woo-search") echo "selected"; ?> ><?php esc_html_e('Search for product','wp-bottom-menu'); ?></option>
	...
	private function repeater_control($value = ''){
		...
		$show_del        = 0; ?>
        <span class="customize-control-title"><?php esc_html_e( 'Social icons', 'wp-bottom-menu' ); ?></span>
		<?php
            ...
            <div class="customizer-repeater-social-repeater">
                <div class="customizer-repeater-social-repeater-container">
                    ...
                    <input type="text" class="customizer-repeater-social-repeater-link"
                           placeholder="<?php esc_attr_e( 'Link', 'wp-bottom-menu' ); ?>">
                    <input type="hidden" class="customizer-repeater-social-repeater-id" value="">
                    <button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'wp-bottom-menu' ); ?>
                    </button>
                </div>
                <input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
            </div>
            <button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'wp-bottom-menu' ); ?></button>
			<?php
		} else { ?>
            <div class="customizer-repeater-social-repeater">
				<?php
                    ...
                    <div class="customizer-repeater-social-repeater-container">
                        ...
                        <button class="social-repeater-remove-social-item"
                                style="<?php if ( $show_del == 1 ) {
							        echo "display:none";
						        } ?>"><?php esc_html_e( 'Remove Icon', 'wp-bottom-menu' ); ?></button>
                    </div>
					<?php
				} ?>
