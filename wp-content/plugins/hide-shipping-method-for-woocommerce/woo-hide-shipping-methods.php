...
if ( !defined( 'WOO_HIDE_SHIPPING_METHODS_VERSION' ) ) {
    define( 'WOO_HIDE_SHIPPING_METHODS_VERSION', '1.3.2' );
}
...
if ( !defined( 'WHSM_PERTICULAR_FEE_AMOUNT_NOTICE' ) ) {
    define( 'WHSM_PERTICULAR_FEE_AMOUNT_NOTICE', 'Enable or disable below advanced hide shipping rules' );
}
...
if ( !function_exists( 'woo_hide_shipping_methods_pro_plugin_path' ) ) {
 ...
 * @return mixed log
 */
if ( !function_exists( 'whsm_main_log' ) ) {
 ...
 *
 */
if ( !function_exists( 'whsm_main_notice_function' ) ) {
 ...
 * @since    1.0.0
 */
if ( !function_exists( 'whsm_plugin_admin_notice_required_plugin' ) ) {
    function whsm_plugin_admin_notice_required_plugin()
    {
        $vpe_plugin = esc_html__( 'Hide Shipping Method For WooCommerce', 'woo-hide-shipping-methods' );
        $wc_plugin = esc_html__( 'WooCommerce', 'woo-hide-shipping-methods' );
        ?>
        <div class="error">
            <p>
                <?php 
        echo  sprintf( esc_html__( '%1$s requires %2$s to be installed & activated!', 'woo-hide-shipping-methods' ), '<strong>' . esc_html( $vpe_plugin ) . '</strong>', '<a href="' . esc_url( 'https://wordpress.org/plugins/woocommerce/' ) . '" target="_blank"><strong>' . esc_html( $wc_plugin ) . '</strong></a>' ) ;
        ?>
            </p>
        </div>
        <?php 
