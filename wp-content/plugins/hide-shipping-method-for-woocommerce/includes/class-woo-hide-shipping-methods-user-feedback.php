 ...
 */
if ( !class_exists( 'HideShipping_User_Feedback' ) ) {
    class HideShipping_User_Feedback
    {
        ...
        public function seconds_to_words( $seconds )
        {
            ...
            if ( $years > 1 ) {
                ...
                return sprintf( __( '%s years', 'woo-hide-shipping-methods' ), $years );
            } elseif ( $years > 0 ) {
                return __( 'a year', 'woo-hide-shipping-methods' );
            }
            ...
            if ( $weeks > 1 ) {
                ...
                return sprintf( __( '%s weeks', 'woo-hide-shipping-methods' ), $weeks );
            } elseif ( $weeks > 0 ) {
                return __( 'a week', 'woo-hide-shipping-methods' );
            }
            ...
            if ( $days > 1 ) {
                ...
                return sprintf( __( '%s days', 'woo-hide-shipping-methods' ), $days );
            } elseif ( $days > 0 ) {
                return __( 'a day', 'woo-hide-shipping-methods' );
            }
            ...
            if ( $hours > 1 ) {
                ...
                return sprintf( __( '%s hours', 'woo-hide-shipping-methods' ), $hours );
            } elseif ( $hours > 0 ) {
                return __( 'an hour', 'woo-hide-shipping-methods' );
            }
            ...
            if ( $minutes > 1 ) {
                ...
                return sprintf( __( '%s minutes', 'woo-hide-shipping-methods' ), $minutes );
            } elseif ( $minutes > 0 ) {
                return __( 'a minute', 'woo-hide-shipping-methods' );
            }
            ...
            if ( $seconds > 1 ) {
                ...
                return sprintf( __( '%s seconds', 'woo-hide-shipping-methods' ), $seconds );
            } elseif ( $seconds > 0 ) {
                return __( 'a second', 'woo-hide-shipping-methods' );
            }
        ...
        public function display_admin_notice()
        {
            ...
            if ( isset( $screen->base ) && 'woocommerce_page_whsm-start-page' === $screen->base ) {
                ...
                $no_bug_url = wp_nonce_url( add_query_arg( $this->nobug_option, 'true', wc_get_current_admin_url() ), 'editorshideshipping-feedback-nounce' );
			...
			<div class="notice updated editorshideshipping-notice">
                ...
                ?>" alt="<?php 
                printf( esc_attr__( '%s WordPress Plugin', 'woo-hide-shipping-methods' ), esc_attr( $this->name ) );
                ?>" />
					</div>
                ...
                ?>
						<h3><?php 
                printf( esc_html__( 'Are you enjoying %s Plugin?', 'woo-hide-shipping-methods' ), esc_html( $this->name ) );
                ?></h3>
						<p>
							<?php 
                ...
                ?>
							<?php 
                printf( esc_html__( 'You have been using %1$s for %2$s now. Mind leaving a review to let us know know what you think? We\'d really appreciate it!', 'woo-hide-shipping-methods' ), esc_html( $this->name ), esc_html( $time ) );
                ?>
						</p>
					</div>
                ...
                printf( '<a href="%1$s" class="button button-primary editorshideshipping-install-button" target="_blank">%2$s</a>', esc_url( $review_url ), esc_html__( 'Leave a Review', 'woo-hide-shipping-methods' ) );
                ?>
						<a href="<?php 
                echo  esc_url( $no_bug_url ) ;
                ?>" class="no-thanks"><?php 
                echo  esc_html__( 'No thanks / I already have', 'woo-hide-shipping-methods' ) ;
                ?></a>
					</div>
				</div>
			</div>
...
new HideShipping_User_Feedback( array(
    ...
    'name'       => __( 'Hide Shipping Method For WooCommerce', 'woo-hide-shipping-methods' ),
