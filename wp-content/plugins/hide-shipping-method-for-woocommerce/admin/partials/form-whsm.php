...
?></label>
                    <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Enable this will hide shipping method from customers', 'woo-hide-shipping-methods' ) ), array(
...
?>
                        ...
                        <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'This can be rule name', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
					</label>
...
?></span></label>
                        <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Shipping method which you want to hide on front side', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
                    </th>
...
?>
				<th class="titledesc" scope="row">
					<label for="fee_settings_shipping_method_list"><?php 
esc_html_e( 'Select shipping method', 'woo-hide-shipping-methods' );
?></label>
                    <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Select shipping method which you want to hide on front side', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
				</th>
...
?></span></label>
                            <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Enter shipping method option with comma separated which you want to hide on front side.(Remove extra space)', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
						</th>
...
?></span></label>
                            <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Apply this extra rule to hide shipping method', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
						</th>
...
?></span></label>
                            <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Select start date on which date you want to hide shipping method.', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
						</th>
...
?></span></label>
                            <?php 
echo  wp_kses( wc_help_tip( esc_html__( 'Select end date on which date you want to hide shipping method', 'woo-hide-shipping-methods' ) ), array(
    'span' => $allowed_tooltip_html,
) ) ;
?>
						</th>
...
?></span></label>
                            <?php 
echo  wp_kses( wc_help_tip( sprintf( wp_kses( __( 'Select days on which day you want to hide shipping method. This rule match with current day which is set by wordpress <a href="%s" target="_blank">Timezone</a>.', 'woo-hide-shipping-methods' ), array(
    'a' => array(
    'href'   => array(),
    'target' => array(),
),
...
?></span></label>
                            <?php 
echo  wp_kses( wc_help_tip( sprintf( wp_kses( __( 'Select time on which time shipping method will hide on the website. This rule match with current time which is set by wordpress <a href="%s" target="_blank">Timezone</a>.', 'woo-hide-shipping-methods' ), array(
    'a' => array(
    'href'   => array(),
    'target' => array(),
),
...
?></h2>
				<div class="tap">
					<a id="fee-add-field" class="button button-large"
					   href="javascript:;"><?php 
...
if ( isset( $sm_metabox ) && !empty($sm_metabox) ) {
    ...
    foreach ( $sm_metabox as $key => $productfees ) {
        ...
        if ( 'cart_total' === $fees_conditions || 'cart_totalafter' === $fees_conditions || 'quantity' === $fees_conditions || 'weight' === $fees_conditions || 'length' === $fees_conditions || 'width' === $fees_conditions || 'height' === $fees_conditions || 'volume' === $fees_conditions ) {
            ?>
        ...
        if ( $i > 2 ) {
            ...
            echo  esc_attr( $i ) ;
            ?>"
									   class="delete-row" href="javascript:void(0);" title="Delete"><i
...
if ( !empty($get_id) && $get_id > 0 && !empty($sitepress) ) {
    $post_obj = get_post( $get_id );
    $wpml_translation = $sitepress->post_translations();
    $discount_metabox = new WPML_Meta_Boxes_Post_Edit_HTML( $sitepress, $wpml_translation );
    ?>
    <div id="icl_div" class="postbox whsma_translation_box">
        <div class="postbox-header whsma_translation_title">
            <h2 class="hndle ui-sortable-handle">Language</h2>
        </div>
        <div class="inside whsma_translation_content">
            <?php 
    $discount_metabox->render_languages( $post_obj );
    ?>
        </div>
    </div>
    <?php 
